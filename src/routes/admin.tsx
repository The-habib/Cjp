...
function AdminPanel() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      ...
      if (u && u.email && (!u.emailVerified || !ALLOWED_EMAILS.includes(u.email))) {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (res.user.email && (!res.user.emailVerified || !ALLOWED_EMAILS.includes(res.user.email))) {
        toast.error("Unauthorized access.");
        await signOut(auth);
      } else {
  ...
  if (!user || (user.email && (!user.emailVerified || !ALLOWED_EMAILS.includes(user.email)))) {
    return (
